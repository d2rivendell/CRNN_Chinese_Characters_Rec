...
"""
用于文本识别中每个batch的训练数据的下标，因为训练数据宽高比不一样，每个batch训练数据的宽度尽量要
一致。padding太多以及图片太宽训练过程中会导致OOM。 padding的比例不高于10%。 有些batch中图片太宽，batch的数量也要
相应降低，这个类就能做到这个功能
"""
class DynamicBatchSampler(data.Sampler):
    def __init__(self, sampler, num_tokens_fn, num_buckets=100, min_size=1, max_size=100,
    ...
    def __len__(self):
		...
		# batch 的大小是动态的，不是常量，尽量不要调用len(dataloader)
        pass
